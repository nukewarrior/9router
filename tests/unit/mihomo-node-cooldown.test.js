...
function route(attemptStartedAtMs = 1000) {
  return {
    proxyProvider: "subscription",
    nodeName: "Example Taiwan Node A",
    egressIdentityKey: "4:198.51.100.20",
    attemptStartedAtMs,
    egressSnapshot: {
...
describe("Mihomo model×egress cooldown", () => {
  it("writes model×egress state and leaves pool status untouched", async () => {
    const pool = makePool();
    ...
    expect(getMihomoModelHealthState(pool, "4:198.51.100.20", "model-a")).toMatchObject({
      ...
      cooldownUntil: "2023-11-14T22:18:20.000Z",
    });
  });

  it("uses exponential cooldown, honors provider reset, and lets newer success reset it", async () => {
    const pool = makePool();
    ...
    const second = await recordMihomoRouteFailure({
      proxyPoolId: pool.id,
      route: route(),
      modelId: "model-a",
      status: 500,
    ...
    expect(getMihomoModelHealthState(pool, "4:198.51.100.20", "model-a")).toMatchObject({
      cooldownUntil: null,
      backoffLevel: 0,
      lastSuccessAt: "1970-01-01T00:33:20.000Z",
    });
  });
  ...
  it("does not cooldown generic 5xx or provider-wide overload", async () => {
    ...
    const overload = await recordMihomoRouteFailure({
      proxyPoolId: pool.id,
      route: route(),
      modelId: "model-a",
      status: 503,
  ...
  it("clears one model×egress cooldown through the mutation boundary", async () => {
    const pool = makePool();
    await recordMihomoRouteFailure({
      proxyPoolId: pool.id,
      route: route(),
    ...
    const cleared = await clearMihomoRouteCooldown({
      proxyPoolId: pool.id,
      route: route(),
      modelId: "model-a",
      mutatePool: mutatorFor(pool),
